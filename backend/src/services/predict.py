...
def generate_recommendations(stress_level: str, top_features: list, activity_data: DigitalActivityInput) -> list:
    """
    Generate rekomendasi berdasarkan tingkat stres dan research-based interventions
    Menggunakan evidence-based digital wellness strategies
    """
    recommendations = []
    
    if stress_level == "Tinggi":
        recommendations.extend([
            "🚨 PRIORITAS TINGGI: Konsultasi dengan profesional kesehatan mental jika diperlukan",
            "🧘‍♀️ Terapkan teknik mindfulness 15-20 menit (terbukti menurunkan kortisol)",
            "📱 Digital detox: Matikan semua perangkat selama 2-3 jam",
            "🏃‍♂️ Aktivitas fisik intensitas sedang 30 menit (endorphin boost)",
            "💤 Prioritaskan tidur 7-9 jam dengan sleep hygiene yang baik",
            "🌿 Teknik grounding: 5-4-3-2-1 (5 hal dilihat, 4 diraba, 3 didengar, 2 dicium, 1 dirasa)"
        ])
        
        # Targeted interventions based on top risk factors
        for feature_name, importance in top_features[:3]:
            if feature_name == "buka_sosmed" and activity_data.buka_sosmed > 3:
                recommendations.append("📵 CRITICAL: Hapus aplikasi media sosial atau gunakan app timer (<1 jam/hari)")
            elif feature_name == "waktu_malam" and activity_data.waktu_malam == 1:
                recommendations.append("🌙 WAJIB: Blue light filter & perangkat off 2 jam sebelum tidur")
            elif feature_name == "notifikasi_count" and activity_data.notifikasi_count > 80:
                recommendations.append("🔕 SEGERA: Turn off ALL non-essential notifications")
            elif feature_name == "durasi_tidur" and activity_data.durasi_tidur < 7:
                recommendations.append("😴 PRIORITAS: Sleep schedule therapy - tidur & bangun jam yang sama")
            elif feature_name == "durasi_pemakaian" and activity_data.durasi_pemakaian > 8:
                recommendations.append("⏰ WAJIB: Batasi screen time maksimal 6 jam dengan break setiap jam")
    
    elif stress_level == "Sedang":
        recommendations.extend([
            "⚠️ Perhatikan pola digital Anda - terapkan strategi preventif",
            "⏰ Gunakan teknik Pomodoro: 25 menit fokus, 5 menit break dari layar",
            "🚶‍♂️ Micro-breaks: Jalan 2-3 menit setiap jam screen time",
            "🎯 Single-tasking: Fokus satu aktivitas digital per waktu",
            "📊 Track usage dengan apps seperti Screen Time atau Digital Wellbeing",
            "🧘‍♀️ Breathing exercises 5 menit, 2x sehari (morning & evening)"
        ])
        
        # Preventive measures based on risk factors
        for feature_name, importance in top_features[:2]:
            if feature_name == "buka_sosmed" and activity_data.buka_sosmed > 2:
                recommendations.append("📱 Batasi sosial media: maksimal 1.5 jam dengan scheduled breaks")
            elif feature_name == "scroll_time" and activity_data.scroll_time > 2:
                recommendations.append("📜 Anti-scroll strategy: Set timer untuk browsing, gunakan reading mode")
            elif feature_name == "notifikasi_count" and activity_data.notifikasi_count > 50:
                recommendations.append("🔔 Kurangi notifikasi: Keep only calls, messages, calendar")
            elif feature_name == "jumlah_aplikasi" and activity_data.jumlah_aplikasi > 12:
                recommendations.append("📲 App minimalism: Uninstall apps yang tidak digunakan >1 minggu")
    
    else:  # Rendah
        recommendations.extend([
            "✅ Pola digital wellness Anda sudah baik - pertahankan keseimbangan ini!",
            "💪 Maintenance mode: Continue current healthy digital habits",
            "📈 Monitor terus: Track weekly digital wellness metrics",
            "🎯 Set boundaries: Tetap konsisten dengan time limits yang ada",
            "🌱 Growth mindset: Explore mindful technology use techniques",
            "� Be a role model: Share healthy digital habits dengan teman/keluarga"
        ])
        
        # Optimization suggestions for healthy users
        if activity_data.durasi_olahraga < 1:
            recommendations.append("🏃‍♂️ Enhancement: Tambah 15 menit aktivitas fisik untuk optimal brain health")
        if activity_data.belajar_online > 0:
            recommendations.append("📚 Optimize learning: Gunakan teknik spaced repetition untuk efisiensi")
    
    # Add universal evidence-based recommendations
    universal_tips = [
        f"� Feature paling berpengaruh: {top_features[0][0].replace('_', ' ').title()}" if top_features else "",
        "🔬 Research tip: 20-20-20 rule (setiap 20 menit, lihat objek 20 kaki, selama 20 detik)",
        "⚡ Quick win: Ganti scrolling dengan physical movement selama 5 menit"
    ]
    
    recommendations.extend([tip for tip in universal_tips if tip])
    
    return recommendations[:10]  # Limit to 10 most relevant recommendations
...
def save_prediction_to_database(user_id: int, activity_data: DigitalActivityInput, prediction_result: dict) -> int:
    ...
    try:
        ...
        digital_activity_id = digital_activity_result['id']
        ...
        prediction_id = prediction_result_db['id']
